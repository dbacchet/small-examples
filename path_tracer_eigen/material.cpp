...
Eigen::Vector3f random_in_unit_sphere() {
    while (true) {
        auto p = Eigen::Vector3f(2*rnd_dis(rnd_gen)-1, 2*rnd_dis(rnd_gen)-1, 2*rnd_dis(rnd_gen)-1);
...
std::optional<Eigen::Vector3f> refract(const Eigen::Vector3f &v, const Eigen::Vector3f &n, float ni_over_nt) {
...
std::optional<Scatter> Lambertian::scatter(const Ray &ray_in, const Eigen::Vector3f &hit_point,
                                           const Eigen::Vector3f &hit_normal) const {
...
std::optional<Scatter> Metal::scatter(const Ray &ray_in, const Eigen::Vector3f &hit_point,
                                      const Eigen::Vector3f &hit_normal) const {
    auto reflected_dir = reflect((ray_in.direction).normalized(), hit_normal) + fuzz_ * random_in_unit_sphere();
...
std::optional<Scatter> Dielectric::scatter(const Ray &ray_in, const Eigen::Vector3f &hit_point,
                                      const Eigen::Vector3f &hit_normal) const {
        auto dn_dot = ray_in.direction.dot(hit_normal);
        auto ray_dir_len = ray_in.direction.norm();
        Eigen::Vector3f outward_normal = -hit_normal;
