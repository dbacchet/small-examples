...
HitableList create_test_scene() {
    HitableList world;
    // Warning: MATERIALS ARE NEVER DEALLOCATED (this is just a toy program after all :)
    world.add(Sphere(Eigen::Vector3f(0.0,0.0,-1.0), 0.5, new Lambertian(Eigen::Vector3f(0.8,0.3,0.3))));
    world.add(Sphere(Eigen::Vector3f(0.0,-100.5,-1.0), 100.0, new Lambertian(Eigen::Vector3f(0.8,0.8,0.0))));
    world.add(Sphere(Eigen::Vector3f(1.0,0.0,-1.0), 0.5, new Metal(Eigen::Vector3f(0.8,0.6,0.2), 0.1)));
    world.add(Sphere(Eigen::Vector3f(-1.0,0.0,-1.0), 0.5, new Dielectric(1.1, 0.0)));
    world.add(Sphere(Eigen::Vector3f(-1.0,0.0,-1.0), -0.45, new Dielectric(1.1, 0.0)));
    world.add(Sphere(Eigen::Vector3f(-1.0,0.5,-1.0), 0.15, new Metal(Eigen::Vector3f(0.1,0.1,0.1), 0.0)));
    return world;
}
...
HitableList create_book_scene() {
    ...
    world.add(Sphere(Eigen::Vector3f(0.0,-1000.0,0.0), 1000.0, new Lambertian(Eigen::Vector3f(0.5,0.5,0.5))));
    // a bunch of small spheres
    for (int a=-11; a<11; a++) {
        for (int b=-11; b<11; b++) {
            float choose_mat = rnd_dis(rnd_gen);
            Eigen::Vector3f center = Eigen::Vector3f(a+0.9*rnd_dis(rnd_gen),0.2,b+0.9*rnd_dis(rnd_gen));
            if ((center - Eigen::Vector3f(4.0,0.2,0.0)).norm() >0.9) {
                if (choose_mat<0.8) { // diffuse
                    world.add(Sphere(center, 0.2, new Lambertian(Eigen::Vector3f(rnd_dis(rnd_gen)*rnd_dis(rnd_gen),
                                                                                          rnd_dis(rnd_gen)*rnd_dis(rnd_gen),
                                                                                          rnd_dis(rnd_gen)*rnd_dis(rnd_gen)))));
                } else if (choose_mat<0.95) {
                    world.add(Sphere(center, 0.2, new Metal(Eigen::Vector3f(0.5*(rnd_dis(rnd_gen)+1.0),
                                                                                     0.5*(rnd_dis(rnd_gen)+1.0),
                                                                                     0.5*(rnd_dis(rnd_gen)+1.0)),
                                                                           0.5*rnd_dis(rnd_gen))));

                } else {
                    world.add(Sphere(center, 0.2, new Dielectric(1.5, 0.1*rnd_dis(rnd_gen))));
                }
            }
        }
    }
    // add the 3 main spheres
    world.add(Sphere(Eigen::Vector3f(-4.0,1.0,0.0), 1.0, new Lambertian(Eigen::Vector3f(0.4,0.2,0.1))));
    world.add(Sphere(Eigen::Vector3f( 0.0,1.0,0.0), 1.0, new Dielectric(1.5,0.0)));
    world.add(Sphere(Eigen::Vector3f( 4.0,1.0,0.0), 1.0, new Metal(Eigen::Vector3f(0.7,0.6,0.5), 0.0)));

    return world;
}

// calculate the color associated to a specific ray
Eigen::Vector3f color(const Ray &ray, const Hitable &world, int32_t depth) {
    ...
    if (hitrecord) {
        ...
        if (scatter) {
            Eigen::Vector3f clr = color(scatter->ray, world, depth+1);
            return Eigen::Vector3f(scatter->color.x() * clr.x(), scatter->color.y() * clr.y(), scatter->color.z() * clr.z());
        } else {
            return Eigen::Vector3f(0.0,0.0,0.0);
        }
    } else {
        ...
        return Eigen::Vector3f(1.0,1.0,1.0)*(1-t) + Eigen::Vector3f(0.5,0.7,1.0)* t;
    }
}
...
pub fn create_test_scene() -> HitableList {
    ...
    world.add(Sphere::new(Vec3::new(0.0,0.0,-1.0), 0.5, new Lambertian(Vec3::new(0.8,0.3,0.3)))));
