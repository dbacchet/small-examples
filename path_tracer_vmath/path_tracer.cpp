...
HitableList create_book_scene() {
    ...
    world.add(Sphere(math::Vector3f(0.0,-1000.0,0.0), 1000.0, new Lambertian(math::Vector3f(0.5,0.5,0.5))));
    // a bunch of small spheres
    for (int a=-11; a<11; a++) {
        for (int b=-11; b<11; b++) {
            float choose_mat = rnd_dis(rnd_gen);
            math::Vector3f center = math::Vector3f(a+0.9*rnd_dis(rnd_gen),0.2,b+0.9*rnd_dis(rnd_gen));
            if (math::length(center - math::Vector3f(4.0,0.2,0.0)) >0.9) {
                if (choose_mat<0.8) { // diffuse
                    world.add(Sphere(center, 0.2, new Lambertian(math::Vector3f(rnd_dis(rnd_gen)*rnd_dis(rnd_gen),
                                                                                          rnd_dis(rnd_gen)*rnd_dis(rnd_gen),
                                                                                          rnd_dis(rnd_gen)*rnd_dis(rnd_gen)))));
                } else if (choose_mat<0.95) {
                    world.add(Sphere(center, 0.2, new Metal(math::Vector3f(0.5*(rnd_dis(rnd_gen)+1.0),
    ...
    world.add(Sphere(math::Vector3f(-4.0,1.0,0.0), 1.0, new Lambertian(math::Vector3f(0.4,0.2,0.1))));
    world.add(Sphere(math::Vector3f( 0.0,1.0,0.0), 1.0, new Dielectric(1.5,0.0)));
    world.add(Sphere(math::Vector3f( 4.0,1.0,0.0), 1.0, new Metal(math::Vector3f(0.7,0.6,0.5), 0.0)));
